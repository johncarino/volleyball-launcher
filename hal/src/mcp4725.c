...
static int fast_write(mcp4725_t *dac, uint16_t value, mcp4725_pd_mode_t pd)
{
    uint8_t buf[2];
...
static int write_dac_reg(mcp4725_t *dac, uint16_t value, mcp4725_pd_mode_t pd)
{
    uint8_t buf[3];
...
static int write_dac_eeprom(mcp4725_t *dac, uint16_t value, mcp4725_pd_mode_t pd)
{
    uint8_t buf[3];
...
int mcp4725_init(mcp4725_t *dac, int bus, uint8_t addr)
{
    ...
    dac->fd = open(dev_path, O_RDWR);
    if (dac->fd < 0) {
        fprintf(stderr, "MCP4725 HAL: cannot open %s: %s\n", dev_path, strerror(errno));
        return -1;
    }

    if (ioctl(dac->fd, I2C_SLAVE, addr) < 0) {
    ...
    if (fast_write(dac, 0, MCP4725_PD_NORMAL) != 0) {
...
int mcp4725_set_raw(mcp4725_t *dac, uint16_t value)
{
    ...
    return fast_write(dac, value, MCP4725_PD_NORMAL);
}
...
int mcp4725_set_throttle(mcp4725_t *dac, int percent)
{
    ...
    return fast_write(dac, mv_to_raw(mv), MCP4725_PD_NORMAL);
}
...
int mcp4725_write_eeprom(mcp4725_t *dac, uint16_t value)
{
    ...
    return write_dac_eeprom(dac, value, MCP4725_PD_NORMAL);
}
...
int mcp4725_set_power_down(mcp4725_t *dac, mcp4725_pd_mode_t mode)
{
    ...
    return write_dac_reg(dac, 0, mode);
}
...
void mcp4725_cleanup(mcp4725_t *dac)
{
    ...
    fast_write(dac, 0, MCP4725_PD_NORMAL);
