...
/* ---- Buzzer output (chip 1, offset 33), 2 kHz square wave ---- */
/* libgpiod v2 API */
...
static void buzzer_cleanup(void) {
    if (buzzer_request) {
...
// Toggles the buzzer GPIO to produce a square wave at BUZZER_FREQUENCY_HZ
// for duration_ms milliseconds, then leaves the line low.
static void buzzer_tone(int duration_ms) {
    if (buzzer_init() != 0) {
        return;
    }

    long total_us = (long)duration_ms * 1000;
    long elapsed_us = 0;
    enum gpiod_line_value level = GPIOD_LINE_VALUE_INACTIVE;

    while (elapsed_us < total_us) {
        level = (level == GPIOD_LINE_VALUE_ACTIVE)
            ? GPIOD_LINE_VALUE_INACTIVE
            : GPIOD_LINE_VALUE_ACTIVE;
        gpiod_line_request_set_value(buzzer_request, BUZZER_GPIO_OFFSET, level);
        usleep(BUZZER_HALF_PERIOD_US);
        elapsed_us += BUZZER_HALF_PERIOD_US;
    }
